...
individual components can be relocated without compromising the system's full functionality. It 
must be possible to build a distributed system, which may require the placement of some components into 
...
selectively for certain types of data. However, if one of the parties demands the other to 
...
To elaborate on [S.A.01](#sa01) and [S.A.02](#sa02) and ensure internal interoperability, 
all communication between components has to happen on HTTPS, in case they don't run in the same 
environment.
...
the goal of the related process. But if data still needs to be transferred, none, other than the data 
...
when it is provided to *data consumers*. Those adjustments must have no impact on the actual 
...
might be required to provide a schema, which should force her to be as precise as possible about 
what data exactly needs to be accessed. In addition to that, the consuming entity must provide some 
*meaningful* text, describing the purpose of the requested data. She should not be allowed to place 
...
All graphical user interfaces must be implemented based on web technologies, which are then 
...
party by either telling her (per URI) where to register or asking her to provide all information 
...
include information about the *consumer*, what data she wants to access, for what purpose, and how 
...
If just one of the user interfaces does not have real-time capabilities but all the others do, the user 
interface might get into an undefined state presenting the user with wrong information, which will 
decrease user experience dramatically. This means, either all user interfaces have to provide 
