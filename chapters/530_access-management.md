...
1)  __Prepare registration__; the *operator* has to tell the third party were and how to register as 
...
feedback channel (e.g. URI) so that the system can get back to that third party. They also may 
...
able to proceed. This approach would short cut and hence simplify the process.*

3)  __Review registration attempt__; the operator gets notified about a new registration attempt, 
which she then has to review and decide whether to accept it or not.

4)  __Create permission profile__; if a *permission request* is enclosed in the registration, it 
has to be reviewed as well. If it's not, step follows immediately. 
...
feedback channel about th decisions and is also provided with further details required to obtain 
actual data.
...
1)  __Send request__; the *data consumer* sends *access request* to the system, containing a all 
...
3)  __Respond to consumer__; handover the computed response back to the requester by proceeding with 
one of the two following options. Either the system responses with a current status of the process 
and where the *consumer* will/can find the demanded data, or the *consumer* includes a callback URI, 
which the system has to invoke with demanded response.
...
connect to the *PDaaS* and has to wrap the *consumer's* own software that is planned to proceed the 
...
*one-time-only*
: access permissions are hereby granted for just a single *access request* (with tolerating certain 
...
enabled in the profile as well. Here the order does matter, because it is imaginable that the 
operation behind (1) is less complex then operation (2). So, at the end running (1) before (2) can 
result in a lower response-time, if operation (1) already results negative. If all operations have a 
...
to somehow benefit from that. So that approach is no option. Using a failure of reliability 
...
Furthermore, it needs to be ensured that only after *permission requests* are being reviewed and 
*permission profiles* are being created, the *data consumers* receive their credentials or a 
...
([S.I.02](#si02)), which can be done in a variety of ways. First, those information can be made 
...
*consumers* only, in order to prevent those information from being crawled. They could be valuable 
as meta data and therefore used in undesired processing that could raise privacy concerns. It is 
imaginable to let the *operators* restrict the access to such availability information on an 
individual consumer-basis or system-wide, and furthermore to set default configurations for this 
behaviour. Depending on those configurations request might fail, thus requester need to be provided 
...
before. The types are `password` and `client_credentials`, which basically require identifier(s) and 
secret or credentials to directly obtain a `token`. The other two types define additional steps 
and interactions involving consumer and operator before getting the `token`, because these other two
types are intended to be used for processes involving untrusted environments (e.g. browser, third 
...
system. The reason to prevent undesired registration attempt, is, because they all have to be
...
process is not needed at all, because according proposal, client identification happens implicitly 
...
Further investigations show that the semantic of an `access_token` from the perspective of an 
...
with the *PDaaS* and the certificate used by the *consumer* is singed by a signature only used for 
that endpoint. This means, the consumer is already authenticated, when the TLS connection has 
successfully established. And since *permission profiles* relate to a specific endpoint, it would 
make providing an `access_token` to become obsolete.
...
solutions are being at disposal:
...
Furthermore in that step does option a) obtain a symmetric key for creating signatures used to ensue 
...
implementations. While this can cause much more harm, it is proposed to leave as much of these 
...
relation. This opens the authorization design up to for example other implementations that might 
be more suitable for certain *data types*. In addition, it would only require little effort to 
...
endpoints, as c) states. So, there is not much benefit in using OAuth, other then developers might 
be somewhat familiar with the API. This can be addressed by a detailed specification for this 
project, hence c) is preferred over b). 
At the end, the only suitable use case from this work would consists of just a request that obtains 
...
How the first steps of a registration are look like, is up to the *consumer*, even though the 
...
When accessing personal data, at the same time preventing those data from leakage is almost 
impossible, which originates in the nature of digital data being able to get effortlessly copied.
...
procedures and introduces therefore new attack scenarios. For now the only approach being 
considered, is the *Supervised Code Execution*, aside from defaulting to simple forwarding. When 
...
provide any dependency at all. The latter is preferred, because it reduces the amount of potentially 
