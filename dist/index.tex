...
different components + en- \& decrypt HTTPS traffic, thus client
authentication + load balancing (if necessary)\\
\emph{Platform:} + server \emph{Technologies:} + HTTP + TLS + WebSockets
\emph{Explanation/argumentation:}

\subparagraph{PKI}\label{pki}

\emph{Purpose:} + CA + manage keys and certificates per \emph{data
consumer} \emph{Platform:} + server \emph{Technologies:} + X.509

\subparagraph{User interface}\label{user-interface}

\emph{Purpose:} + access \& permission management + data management
(editor, types \& import) + history log \emph{Platform:} + desktop +
mobile \emph{Technologies:} + HTML, CSS, Javascript + Java + Swift,
Objective-C

\subparagraph{Personal Data Storage}\label{personal-data-storage}

\emph{Purpose:} \emph{Platform:}\\
\emph{Technologies:} + regardless of the platform + connector + al least
two storage: one for the actual data, the other for permission profiles,
history and all the other application data - where to place the storage?
local (e.g.~mobile device) or cloud (e.g.~hoster's infrastructure) +
requires 24/7 uptime

\subparagraph{Persistence}\label{persistence-1}

\emph{Purpose:} + stores Permission Profiles, History, Tokens,
Configurations and other meta data \emph{Platform:}\\
\emph{Technologies:}

\subparagraph{Domain Logic}\label{domain-logic}

\emph{Purpose:} + permission verification \emph{Platform:}\\
\emph{Technologies:}
...
\emph{Purpose:} \emph{Platform:}\\
\emph{Technologies:} + websockets for web UIs + Google/Apple
Notification server compatible connection for mobile apps

\subparagraph{Data API}\label{data-api}

\emph{Purpose:} \emph{Platform:}\\
\emph{Technologies:} + essentially consists of two parts: 1) checking
permissions of the request 2) persistence layer abstraction (graphql) +
for external consumers - incoming permission requests and data access
attempts - outgoing data () + for internal clients (web UI, mobile
device)
...
\subsection{Data API}\label{data-api-1}
