...
towards the original web shop entry. Finally she comes around and puts
the item onto her shopping cart, despite the fact that she never bought
...
values are really the ones that belong to exactly they individual they
...
narrative that requests happen here and there and are preventable. But
when it comes to native mobile applications, it's almost impossible
...
seems to cause that \emph{OAuth} is being misleadingly - but
...
distributed or a centralized manner. The outcome of this section should
...
One is a rather centralized approach and the other involve more platform
...
developed solutions. Hence parts of the contents might reoccurring.
...
\texttt{CONSUMER\_ID.system.tld}). Further components are i.a. a
...
\item
  handover results back to \emph{Permission Manager}
\item
...
\item
  scan third party registrations via QR-Codes or generate URI to submit
  registration
\end{enumerate}
...
All communications from and towards the system as well as internal
...
\emph{RSA} SHOULD be preferred over \emph{DSA}. All created keys MUST
have a length of at least \texttt{4096} bits. The Ciphers for TLS, that
support \emph{Perfect Forward Secrecy} SHOULD be preferred, but ciphers
not supported by \texttt{TLSv1.2} MUST be avoided. Evey endpoint and all
...
system and thus get served by the \emph{web server} as well. This
required behaviour is enforced by utilizing the \emph{Content Security
Policy (CSP)} in HTTP headers. The \emph{web server} MUST facilitate a
web socket connection fFor web-based GUIs. If a browser does not support
those natively, a fallback SHALL be provided by the GUI. Furthermore,
those GUIs SHOULD be served with HTTP/2.
...
email or if a mobile platform is part of the system.
...
transferred to the consumer on a secure channel, which the consumer is
responsible to provide (e.g.~HTTP over TLS certified by trusted public
CA, or by a self-signed certificate provided with the registration).

In order to use TLS for bidirectional authentication, not only the
client (consumer) MUST be able to verify the server's (system)
certificate, also the server MUST do the same for the client. This
...
the authentications has failed, and vice versa the consumer has
...
least after half of the period of validity has been reached. The period,
in which the token is valid, SHOULD be 24 hours but MUST NOT exceed 48
hours.
...
which later is fetched onto a front end platforms, initially is stored
...
platform, that might as well be used by the operator to manage the
system. The general approach of a more distributed deployment of
components SHOULD reduce the vulnerability to certain scenarios and make
it harder for entities to compromise (parts of) the system. As long as
all requirements are met and every component is completely functional as
well as the system as a whole, any component MAY be located on whatever
platform is sufficient.

A second approach to gain not only portability, but also to increase the
overall level of security is isolate components {[}and their
...
environment, namely \emph{cgroups} and \emph{systemd-nspawn}, or by
putting components into application containers. The latter is
RECOMMENDED and MUST respect all \emph{Open Container} Specifications.
An orchestration software MAY be utilized to manage all containers.
...
is REQUIRED to only execute them after putting them into an application
container (as referred to in
\protect\hyperlink{system-architecture}{System Architecture}). This
implies to first provision the container and then invoke it by providing
the requested data points as parameters. The container MUST NOT be
...
automatically be inspected and reviewed. If one of those test layers
result insufficiently, processing the access request MUST abort and MUST
...
get persisted \emph{(required fields for those information are defined
in \protect\hyperlink{data-and-types}{Data and Types})}:
...
\item
  results of operator verifications
\item
  Permission Profile creation, manipulation and deletion
\item
  supervised code execution, regardless of its success
\item
  any other request attempt reaching the web server
\item
...
server}, MUST push the information towards the \emph{Tracker}. By
performing pattern recognition \& anomaly detection, the \emph{Tracker}
is then able to recognize abnormal behaviour/occurrence, for example by
...
such, but any other notation is explicit}
...
\item
  ...
  \item
    ...
    \item
      \texttt{type:\ String} - access type (\texttt{sce},\texttt{cp})
    \item
      \texttt{query:\ String} - data query
    \item
    ...
    \item
      \texttt{result}: result of SCE {[}OPTIONAL{]}
    \end{itemize}
  \item
    access-validation
    ...
    \tightlist
    \item
      \texttt{access-request:\ String} - reference to access request
    \item
      \texttt{reason:\ String} - why has the verification failed
    \end{itemize}
  \end{itemize}
\end{itemize}
