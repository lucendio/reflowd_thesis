...
\textbf{REST(ful)}\footnote{\emph{Representational State Transfer},
  ...
  {[}\protect\hyperlink{ref-web_spec_rest}{88}{]}} is a common set of
...
meanings, it involves a group of rudimentary vocabulary\footnote{knows
  ...
  {[}\protect\hyperlink{ref-web_spec_http-methods}{89}{]} (e.g.~GET,
...
{[}\protect\hyperlink{ref-web_spec_graphql}{90}{]} stands for
...
standards have a certain relevance to that concept. RDF\footnote{Resource
  Description Framework {[}\protect\hyperlink{ref-web_w3c-tr_rdf}{91}{]}}
basically defines the syntax. OWL\footnote{Web Ontology Language
  {[}\protect\hyperlink{ref-web_w3c-tr_owl}{92}{]}} provides the
guidelines on how the semantics and schemas should be defined and with
SPARQL {[}\protect\hyperlink{ref-web_w3c-tr_sparql}{93}{]}, the query
...
{[}\protect\hyperlink{ref-web_w3c-draft_webid}{94}{]}} - and provided in
...
Linux kernel. \emph{Cgroups},\footnote{control groups
  {[}\protect\hyperlink{ref-web_2015_cgroup-doc}{98}{]}} which define or
...
{[}\protect\hyperlink{ref-web_2016_kernel-namespace}{99}{]} define or
...
{[}\protect\hyperlink{ref-web_2016_open-container-initiative}{100}{]},
...
{[}\protect\hyperlink{ref-web_oci-spec_runtime}{101}{]} and image
{[}\protect\hyperlink{ref-web_oci-spec_image}{102}{]} definition, on
...
Promoted from the data subject to the data owner, being the center of
the \emph{PDaaS}, the operator gains abilities to have as much control
over all the data related to her as possible, to determine in a very
precise ways what data of hers can be accessed by third parties and to
...
{[}\protect\hyperlink{ref-book_2015_ethical-it-innovation}{103}{]}, then
...
{[}\protect\hyperlink{ref-paper_2004_distributed-mapreduce}{104}{]}. The
...
option exists to preserve the goal of that process.
...
most appropriate technology to communicate with the \emph{PDaaS} over
the internet would be \emph{\protect\hyperlink{link_http}{HTTP}}. This
comprehensive and flexible protocol allows several technologies to use
for server-client authentication purposes. Within the scope of this
work, those technologies are categorized in the following types (TODO:
maybe find other labels): (A) stateful and (B) stateless authentication.
The first one (A) includes \emph{Basic access Authentication} (or
\emph{Basic Auth}) and authentication based on \emph{Cookies}. Whereas
the \emph{two-way authentication}
{[}\protect\hyperlink{ref-web_2017_wikipedia_mutual-auth}{105}{]} in TLS
{[}\protect\hyperlink{ref-web_spec_tls-12_client-auth}{106}{]} and
\protect\hyperlink{link_jwt}{authentication based on web-token} are
associated with the later category (B). \emph{Basic Auth} is natively
provided by the \emph{http-agent} and requires, in it's original form
(\emph{user:password}), some sort of state on the server; at least when
the system should provide multitenancy. If instead just a general access
restriction for certain requests would suffice, no state is required.
One of the most common implementations of server-side states is a
\emph{session} that contains one or more values representing the state
and a unique identifier, by which an entity can be associated with. Such
a session ID is typically provided as part of the HTTP header, whether
as \emph{Basic Auth} value, a \emph{Cookie}, which is domain-specific,
or some other custom header.
...
First of all, two different types of motivations for authenticating to
the system have to be distinguished. First of all, two different types
of motivations have to be distinguished for authenticating to the
system. One, originated by an external third party (data consumer), is
to make a permission request or to access data, the other is

In other words, it would make sense to distinguish the authentication
process of the two different \protect\hyperlink{sa03}{roles}, because
the way these two are interacting with the system is different,
otherwise there would be no two roles
...
In a public-key-based authentication when a receiver is able to decrypt
the incoming request itself it implies that the requester's identity is
valid and the integrity of the containing data is given. Whereas on a
token-based authentication every incoming request has to carry the token
so that the system can verify and associate the request with an account
and the data it not automatically encrypted and thus integrity is not
preserved.
...
\emph{e-government}.]{\texorpdfstring{ePerso, E-Post/de-mail as part of
a PKI solution?\_\_\\
In the past years different countries around the world recently started
to introduced \emph{information technology} to the day-to-day processes,
interactions and communications between public services and their
...
developments is the so called \emph{electronic ID card}, hereinafter
...
\emph{e-government}.}{ePerso, E-Post/de-mail as part of a PKI solution?\_\_ In the past years different countries around the world recently started to introduced information technology to the day-to-day processes, interactions and communications between public services and their citizens, for example changing residence information or filing tax report, which is summarized under the term E-government. One of those developments is the so called electronic ID card, hereinafter called eID card. Equipped with storage, logic and interfaces for wireless communication, those eID cards can be used to store certain information and digital keys or to authenticate the owner electronically to a third party without being physically present. Such an eID card was also introduced in Germany in 2010. The so called nPA was an important step towards an operational e-government.}}\label{eperso-e-postde-mail-as-part-of-a-pki-solution__-in-the-past-years-different-countries-around-the-world-recently-started-to-introduced-information-technology-to-the-day-to-day-processes-interactions-and-communications-between-public-services-and-their-citizens-for-example-changing-residence-information-or-filing-tax-report-which-is-summarized-under-the-term-e-government-abbr_egov.-one-of-those-developments-is-the-so-called-electronic-id-card-hereinafter-called-eid-card.-equipped-with-storage-logic-and-interfaces-for-wireless-communication-those-eid-cards-can-be-used-to-store-certain-information-and-digital-keys-or-to-authenticate-the-owner-electronically-to-a-third-party-without-being-physically-present.-such-an-eid-card-was-also-introduced-in-germany-in-2010.-the-so-called-npa-abbr_npa-was-an-important-step-towards-an-operational-e-government.}
...
{[}\protect\hyperlink{ref-web_bsi-spec_eid}{109}{]} starting with making
...
{[}\protect\hyperlink{ref-web_2017_npa-eid-server}{110}{]} to sign a
...
required for creating a \emph{QES}\footnote{Qualified Electronic
  Signatures
  {[}\protect\hyperlink{ref-paper_2013-keymanangement-fuer-qes-mit-npa}{111}{]}}
...
{[}\protect\hyperlink{ref-web_2017_about-de-mail}{112}{]}. It's an
...
{[}\protect\hyperlink{ref-statement_2013_de-mail}{113}{]}, maybe even
...
\item
  ...
  {[}\protect\hyperlink{ref-web_2009-success-of-facebook-connect}{114}{]}.
\end{itemize}
...
{[}88{]} T. Fielding, ``Representational state transfer (REST),'' in
...
{[}89{]} P. J. Leach, T. Berners-Lee, J. C. Mogul, L. Masinter, R. T.
...
{[}90{]} ``GraphQL,'' Oct-2016. {[}Online{]}. Available:
\url{https://facebook.github.io/graphql/}. {[}Accessed: 17-Dec-2016{]}

\hypertarget{ref-web_w3c-tr_rdf}{}
{[}91{]} D. Beckett and B. McBride, ``RDF/XML syntax specification
...
{[}92{]} W. O. W. Group, ``OWL 2 web ontology language document overview
...
{[}93{]} S. Harris, A. Seaborne, and E. Prud'hommeaux, ``SPARQL 1.1
...
{[}105{]} ``Mutual authentication,'' 02-Sep-2016. {[}Online{]}.
Available:
...
{[}106{]} T. Dierks, ``The transport layer security (TLS) protocol
...
{[}109{]} ``BSI - technische richtlinien des BSI - BSI TR-03130
...
{[}110{]} ``Personalausweisportal - eID-server.'' {[}Online{]}.
Available:
...
{[}111{]} K. Nguyen and C. Schwarz, ``Innovatives key management für die
...
{[}112{]} D. B. der Bundesregierung für Informationstechnik,
...
{[}113{]} L. Neumann, ``Stellungnahme zum elektronischen
...
{[}114{]} N. Carlson, ``Facebook connect is a huge success -- by the
