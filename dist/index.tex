...
{[}\protect\hyperlink{ref-web_spec_tls-12_client-auth}{107}{]} it is
...
authentication}
{[}\protect\hyperlink{ref-web_2017_wikipedia_mutual-auth}{108}{]}) is
...
Resumption}
{[}\protect\hyperlink{ref-book_2013_networking-101_tls-session-resumption}{109}{]},
...
Resumption}
{[}\protect\hyperlink{ref-web_spec_tls-session-ticket-resumption}{110}{]}
...
ensure integrity and perhaps also the confidentiality only of their own
carriage and not the entire HTTP payload, both requirements need to be
addressed separately. Serving HTTP over \emph{TLS} solves this issue,
but using TLS or \emph{asymmetric cryptography} properly - place value
on integrity, confidentiality, authenticity - requires additional
infrastructure. Such an infrastructure is known as \emph{Public Key
Infrastructure} (or \emph{PKI})
{[}\protect\hyperlink{ref-book_2014_chapter-14-5-pki}{111}{]}. It
manages and provides public keys in a directory, including related
information to the entities those certificates belong to. A Certificate
Authority (or \emph{CA}), as part of that infrastructure, issues,
maintains and revokes digital certificates. The infrastructure that is
needed to provide secure HTTP connections for the internet can be seen
as a large, if not the largest, public \emph{PKI}. It is based on the
...
{[}\protect\hyperlink{ref-web_spec_x509}{112}{]}. For connections that
use a web token, it should be sufficient to rely on the public PKI that
drives \emph{HTTP} over \emph{TLS}, because all information required to
authenticate is provided by the token itself. Though, the situation is
different if instead \emph{two-way authentication} is used. For this,
the system has to provide it's own \emph{PKI} including a Certificate
Authority that issues certificates for \emph{data consumers}, because
not only the \emph{endpoints} on the \emph{PDaaS} (server) need to be
certified, all \emph{data consumers} (clients) need to present a
certificate as well. Only the \emph{PDaaS} verifies and thus determines
(supervised by the \emph{operator}) who is authorized to get access to
the system. Hence the \emph{PKI} needs to be self-contained without any
external role in order to function independently so that only invited
parties can get involved.
...
same time. Or the account, a token is associated with, can actually have
...
what purposes, the private key needs to be on those clients. But a
private key typically does not leave it's current system or is at least
not used multiple times, in order to prevent exposure, which is implied
in any action of duplication. To reduce those risks, it's common
practice to generate a private key at that location where it is going to
be used.
...
{[}\protect\hyperlink{ref-book_2014_chapter-10-5-asym-random-number-gen}{113}{]},
...
advantages of the two authentication mechanisms, it would make sense to
use asymmetric cryptography in combination with \emph{HTTPS} for the
...
\section{Data Authenticity}\label{data-authenticity}

Whether it's the \emph{operator} or a \emph{data consumer}, not only the
entity's identity has to be authenticated when communicating with the
system. The actual personal data being used has to be authentic and
correct as well (\protect\hyperlink{sa04}{S.A.04}). This results in the
following required scenario: an trustworthy third party has to look at
the existing data in the database and then compare them with the actual
data that's used to create a response to a request made by a \emph{data
consumer}. If the result is positive, meaning both data sets are equal,
the entity has to verifies this somehow. The response then indicates to
the \emph{data consumer} that authenticity state, which is determined by
that third party.

The described scenario is the direct opposite to the previous
argumentation.
...
\subsection[ePerso, E-Post/de-mail as part of a PKI solution?\_\_\\
...
{[}\protect\hyperlink{ref-web_2013_npa-sicherheitsdefizit}{114}{]} and
disadvantages
{[}\protect\hyperlink{ref-web_2014_test-qes-support-in-npa}{115}{]} an
...
{[}\protect\hyperlink{ref-web_bsi-spec_eid}{116}{]} starting with making
...
{[}\protect\hyperlink{ref-web_2017_npa-eid-server}{117}{]} to sign a
...
required for creating a \emph{QES}\footnote{Qualified Electronic
  Signatures
  {[}\protect\hyperlink{ref-paper_2013-keymanangement-fuer-qes-mit-npa}{118}{]}}
...
{[}107{]} T. Dierks, ``The transport layer security (TLS) protocol
...
{[}108{]} ``Mutual authentication,'' 02-Sep-2016. {[}Online{]}.
Available:
...
{[}109{]} ``Networking 101: Transport layer security (TLS) - high
...
{[}110{]} P. E. Joseph Salowey H. Zhou, ``Transport layer security (TLS)
...
{[}111{]} W. Stallings, ``10.5 pseudorandom number generation based on
an asymmetric cipher,'' in \emph{Cryptography and network security:
Principles and practice}, Seventh edition., Boston: Pearson, 2014, pp.
443--445.

\hypertarget{ref-web_spec_x509}{}
{[}112{]} D. Cooper, S. Santesson, S. Farrell, S. Boeyen, and W. Housley
...
{[}113{]} W. Stallings, ``9.1 public-key infrastructure,'' in
...
{[}114{]} ``Basisleser weiterhin kritische schwachstelle des
...
{[}115{]} O. Stiemerling, ``Qualifizierte elektronische signatur mit dem
...
{[}116{]} ``BSI - technische richtlinien des BSI - BSI TR-03130
...
{[}117{]} ``Personalausweisportal - eID-server.'' {[}Online{]}.
Available:
...
{[}118{]} K. Nguyen and C. Schwarz, ``Innovatives key management für die
...
{[}119{]} D. B. der Bundesregierung für Informationstechnik,
...
{[}120{]} L. Neumann, ``Stellungnahme zum elektronischen
