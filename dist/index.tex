...
that certificate. This is, \emph{consumers} need to be provided with
...
on, it describes some basic data types and structs, that might be useful
in the context of \emph{personal data} as well as for previously
introduces \protect\hyperlink{scenarios}{scenarios}.

First of all, to address the need of portability related to those
components, storing and providing \emph{personal data}, it is essential
to abstract the actual storage from how it gets accessed. This makes it
possible to relocate those storage into other platforms and
environments. Thereby the \emph{personal data storage} itself become
platform-agnostic, in other words portable. In order to reduce possible
...
TODO: Technology: \emph{GraphGL} or the \emph{Semantic Web Suite} ???

responsible for persisting any kind of data

to elaborate on the different mechanisms to save and store certain
groups of system-relevant data

Both languages know the concept of a graph, which means relations
between data points are embedded in the data structure itself. This is,
in terms of a graph, relations are \emph{edges} and data points are
\emph{nodes}. In consequence the structure of a query shows the same as
the result does, which means the originator of that query knows exactly
        ...
        \DataTypeTok{"bindings"}\FunctionTok{:} \OtherTok{[}
            \FunctionTok{\{}
                \DataTypeTok{"accountId"}\FunctionTok{:} \FunctionTok{\{}
                    \DataTypeTok{"type"}\FunctionTok{:} \StringTok{"literal"}\FunctionTok{,}
                    \DataTypeTok{"value"}\FunctionTok{:} \StringTok{"janeod23"}
                \FunctionTok{\},}
...
The syntax itself already shows its nature of decentralization, which at
the same time introduces additional external dependencies. Because the
query language itself has no concept of schemas, structs or any kind of
semantic, it needs to be made aware of them.

SPARQL queries typically return XML which then can be rendered into
(HTML) tables, but JSON and RDF are supported as well.
...
\FunctionTok{\{}
    ...
    \DataTypeTok{"bankAccounts"}\FunctionTok{:} \OtherTok{[}
        \FunctionTok{\{}
            \DataTypeTok{"accountId"}\FunctionTok{:} \StringTok{"janeod23"}\FunctionTok{,}
...
Whereas comparing the GraphQL query syntax with it's result shows of a
remarkable resemblance.

While SPARQL has a full-featured query language syntax including all
sorts of logic (e.g. aggregation, operators, nested queries etc.),
GraphQL's syntax on the other hand is more rudimental. All it's
functions and logic is separated from the language itself into a
backend-like part, that is responsible for resolving queries. Those
resolvers then implement features like sorting, searching, grouping or
aggregating. With this concept of separation it is straightforward to
validate queries, because it is essentially matching against types.

Both share a comprehensive understanding of a type-system, that
encourages to create all kinds of data types.

\begin{itemize}
\item
  keep in mind to make it all somehow extensible, e.g.~by using and
...
{[}\protect\hyperlink{ref-web_2016_wikipedia_separation-of-concerns}{125}{]}.
...
architecture can be placed}\tabularnewline
\toprule
...
\item
  X.509
\item
  ACME {[}\protect\hyperlink{ref-web_spec_acme}{126}{]} (Let's Encrypt)
\end{itemize}
...
some redundancy concept. A first solution would be regular device
backups. Other solutions are possible but not in the scope of this work.
A mobile platform being part of the system makes it more easier for the
\emph{data subject} to establish a security concept, in which the
relation the between personal data storage and the rest of the system is
...
information, or put it into the \emph{personal data storage} as well or
define it as an own storage component, in order to be flexible regarding
it's location.
...
that, this separation can be further brought simply by encapsulating
...
parties
{[}\protect\hyperlink{ref-web_2009-success-of-facebook-connect}{127}{]}
...
\emph{Conclusions:} Considering amount of effort a single-platform
version, namely client or mobile, would take to get fully operational
with respect to the specification, it is not only reasonable but also
more secure to involve a server environment with proper security
measures, static IP and high availability. Even if that server is a
local machine connected to the \emph{data subject's} private network.
That said, it is sufficient to start with the \emph{monolithic} approach
and as suitable mobile applications emerge that are supporting major
administration features, notifications and \emph{personal data storage},
it should be possible to migrate effortlessly towards the
...
as stated in the project's core principles Open Development is vital.
Interestingling this has a significant effect in how a \emph{PDaaS}
might be deployed or installed. All components can just be taken and
used as it fits everyone's needs of cause, with respect to their
licenses.

the portability enables independent development of the components, as
well as what technologies are being used for them.
...
\item
  domain, new one preferred or with no subdomains used; ability to
  change the domain's DNS configuration
\item
  migrating from server-located \emph{personal data storage} to the
  mobile based version. how?
\end{itemize}
...
{[}125{]} \emph{Separation of concerns}. URL
...
{[}126{]} \textsc{Kasten, James}\,; \textsc{Barnes, Richard}\,;
...
{[}127{]} \textsc{Carlson, Nicholas}: \emph{Facebook connect is a huge
