...
manually or automatically enrich content (e.g.geo-location attached to a
tweet as meta information). Moreover, it refers to data that is captured
by someone or something from within the individual's private living
space or property. \emph{Personal data} is basically understood as every
data point reflecting the individual as such and her personality -
...
Despite those two exceptions, elaborations on \emph{data ownership} are
almost non-existant in current legislation. Instead, the question is
...
data. As mentioned above, personal information are not the subject of
...
engagement.''} These principles can be found in various forms across
much work done in this area of research.
...
\tightlist
\item
  a code-based representation of that question, which gets executed in a
  sandbox on the data subjects's \emph{PDS} system with the required
  data as arguments. The data, used as arguments, is implicitly defined
  trough (A). The output of that execution represents both answer and
  response.
\end{enumerate}
...
It will be interesting to see how these two related but distinct
...
{[}\protect\hyperlink{ref-web_w3c-draft_webid}{109}{]}} - and provided
...
\textbf{Solid}.\footnote{social linked data
  {[}\protect\hyperlink{ref-web_spec_solid}{110}{]}} Backed by the
\emph{WebAccessControl}
{[}\protect\hyperlink{ref-web_2016_wiki_webaccesscontrol}{111}{]} system
...
{[}\protect\hyperlink{ref-web_2016_demo_databox}{112}{]} follows the
...
minor flaws
{[}\protect\hyperlink{ref-web_2013_npa-sicherheitsdefizit}{118}{]} and
disadvantages
{[}\protect\hyperlink{ref-web_2014_test-qes-support-in-npa}{119}{]} an
...
{[}\protect\hyperlink{ref-web_2017_about-de-mail}{120}{]}. It's an
...
{[}\protect\hyperlink{ref-statement_2013_de-mail}{121}{]}; maybe even
...
{[}\protect\hyperlink{ref-statement_2013_de-mail}{121}{]}, who have been
...
agent''}
{[}\protect\hyperlink{ref-book_2015_ethical-it-innovation}{122}{]}, then
...
requesting source; regardless of whether it's during initial
...
{[}\protect\hyperlink{ref-web_oci-spec_image}{117}{]}. This concept can
...
\addcontentsline{toc}{subsection}{Scenarios \emph{(Excerpt)}}
...
\addcontentsline{toc}{subsection}{Architectural Overview}
...
\addcontentsline{toc}{subsection}{General Process Description}
...
\addcontentsline{toc}{subsection}{Relations}
...
\addcontentsline{toc}{subsection}{Depending Technologies and Standards}
...
\addcontentsline{toc}{subsection}{Transport}
...
The subsequent examples show two Nginx configuration for the \emph{web
server}, implementing the previous specifications.
...
\addcontentsline{toc}{subsection}{Authentication}
...
\paragraph{Transport Layer Security}\label{transport-layer-security}
...
\paragraph{JSON Web Token (JWT)}\label{json-web-token-jwt}
...
\item
  ...
  \item
    ...
    \item
      ...
      is requested to get permission to access, those information MUST
...
\addcontentsline{toc}{subsection}{Personal Data}
...
\addcontentsline{toc}{subsection}{System Data}
...
\addcontentsline{toc}{subsection}{Data Models for System Data}
...
Defaults\_\_}\label{system-configurations-and-defaults__}
...
mobile platforms, but at least MUST be provided for \emph{desktop}
...
\item
  utilize \emph{accordion} pattern (re)viewing data structures
\item
  model new data structures
\item
  migrate \emph{PDS} to another platform
\end{itemize}
...
hosted by the system. The payload MUST be transferred with HTTP
requests, secured by TLS, as \emph{POST} method, unless otherwise
outlined. It MUST be serialized in JSON and constitutes the complete
request body. In order to send data in certain formats
(e.g.~PEM-formatted file content or binary streams) as a string values
as part of a URLs or JSON, \emph{bas64url} MUST be used to encode such
data. Furthermore, it is to be noted, that request are asynchronous and
MAY take several hours up to a few days to get answered.

\emph{NOTICE: If the representation of the data that is requested for
access defined as a list of strings (\texttt{{[}String{]}}), then the
response has the same structure. Same apply to the data query in a
string-representation (\texttt{String}).}
...
web page, which is served via HTTPS by the requester, ready to get
scanned with the operator's mobile device.
...
\item
  ...
  {[}OPTIONAL{]} list of endpoint selectors or query string (see
...
\begin{Highlighting}[numbers=left,,]
\FunctionTok{\{}
    \DataTypeTok{"endpoint"}\FunctionTok{:} \StringTok{"https://$consumerId.system.tld"}\FunctionTok{,}
...
\item
  ...
  of endpoint selectors or query string, representing what data is
  requested to get access to
\end{itemize}

\emph{Request:} \texttt{https://\$consumerId.system.tld/pr}
...
\item
  \texttt{type:\ String} - access type: \texttt{fwd} or \texttt{sce}
\item
  ...
  \end{itemize}
\item
  \texttt{reliable:\ Boolean} - indicates whether the consumer expects a
  proof of data reliability or not
\item
  \texttt{respond:\ String} - \texttt{push} (returns immediately with
  information where to obtain the result) or \texttt{keepalive} (keep
  connection open until response is computed)
\end{itemize}

\emph{Request:} \texttt{https://\$consumerId.system.tld/ar}
...
\item
  \texttt{duration:\ Number} - time in seconds until result SHOULD be
  available
\end{itemize}
...
\begin{Highlighting}[numbers=left,,]
\FunctionTok{\{}
    \DataTypeTok{"pickup"}\FunctionTok{:} \StringTok{"https://$consumerId.system.tld/ar/$procedureId"}\FunctionTok{,}
...
\emph{Request:}
\texttt{https://\$consumerId.system.tld/ar/\$procedureId}
\emph{Response:}
...
\item
  \texttt{proof:\ String} - certificate that indicates verified data
  reliability
\item
...
This section provide additional information on hosting and operating
...
\item
  ...
  recommended to run other applications on the same environment side by
  side.
\item
  a vendor, who hosts for multiple data subjects, one system instances,
  SHOULD NOT locate all of them in one environment, but rather deploy a
  separate virtualization of an operating system for every instance.
\item
  the host environment (for the components) SHOULD install measures for
  detecting security violations and breaches
\end{itemize}
...
\item
  ...
  representing different individuals, it MAY considerable to use another
  key-pair and certificate for any of those instances, and maybe even
  unique endpoints.
\item
...
{[}\protect\hyperlink{ref-web_2017_repo_node-simple-schema}{147}{]}.}
...
{[}106{]} \textsc{Beckett, Dave}\,; \textsc{McBride, Brian}:
...
{[}107{]} \textsc{W3C OWL Working Group}: \emph{OWL 2 web ontology
...
{[}108{]} \textsc{Harris, Steve}\,; \textsc{Seaborne, Andy}\,;
...
{[}118{]} \emph{Basisleser weiterhin kritische schwachstelle des
...
{[}119{]} \textsc{Stiemerling, Oliver}: \emph{Qualifizierte
...
{[}120{]} \textsc{Bundesregierung für Informationstechnik, Der
...
{[}121{]} \textsc{Neumann, Linus}: Stellungnahme zum elektronischen
rechtsverkehr.

\hypertarget{ref-book_2015_ethical-it-innovation}{}
{[}122{]} \textsc{Spiekermann, Sarah}: \emph{Ethical IT Innovation: A
Value-Based System Design Approach}~: CRC Press; Taylor \& Francis
Group, LLC, 2015 --~scale ---~ISBN~978-1-4822-2635-5

\hypertarget{ref-paper_2004_distributed-mapreduce}{}
{[}123{]} \textsc{Dean, Eeffrey}\,; \textsc{Ghemawat, Sanjay}:
...
{[}147{]} \emph{Aldeed/node-simple-schema. GitHub}. URL
