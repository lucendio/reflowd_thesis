...
When using JWT, \texttt{\{"alg":"none"\}} as header MUST NOT be used. A
JWT MAY be encrypted (JWE). If the operator fails to connect to the
...
least after half of the period of validity has been reached. JWTs MUST
be created and verified by the \emph{Operator API}. The secrets and keys
for that purpose are stored in the \emph{Persistence Layer}. Signature
validation MAY get performed by the \emph{Web server}.
...
\subsection{Supervised Code Execution}\label{supervised-code-execution}
...
\tightlist
\item
  pattern recognition \& anomaly detection for access requests
  (e.g.~check if request come constantly from the same IP)
