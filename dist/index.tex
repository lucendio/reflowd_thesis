...
straightforward to implement into the \emph{PDaaS}. And since web tokens
...
use a web token, it is sufficient to rely on the public PKI that drives
\emph{HTTP} over \emph{TLS}, because unlike the \emph{two-way
authentication}, authentication is provided by the token itself. The
situation is different, if instead \emph{two-way authentication} is
used. For this the system has to provide it's own \emph{PKI} including a
Certificate Authority that issues certificates for \emph{data
consumers}.
...
\emph{mutual authentication} the client's private key is required. Such
key should not leave it's current place in order to prevent exposure,
which implies any action of duplication.

If a public-key-based connection, performing a \emph{mutual
authentication}, establishes successfully, it implies that the
...
request with an account. Data it not automatically encrypted and thus
...
payload containing personal data, parameters for encryption procedures
...
{[}\protect\hyperlink{ref-web_2013_npa-sicherheitsdefizit}{113}{]} and
disadvantages
{[}\protect\hyperlink{ref-web_2014_test-qes-support-in-npa}{114}{]} an
...
{[}\protect\hyperlink{ref-web_bsi-spec_eid}{115}{]} starting with making
...
{[}\protect\hyperlink{ref-web_2017_npa-eid-server}{116}{]} to sign a
...
required for creating a \emph{QES}\footnote{Qualified Electronic
  Signatures
  {[}\protect\hyperlink{ref-paper_2013-keymanangement-fuer-qes-mit-npa}{117}{]}}
...
{[}\protect\hyperlink{ref-web_2017_about-de-mail}{118}{]}. It's an
...
{[}\protect\hyperlink{ref-statement_2013_de-mail}{119}{]}, maybe even
...
{[}111{]} W. Stallings, ``9.1 public-key infrastructure,'' in
\emph{Cryptography and network security: Principles and practice},
Seventh edition., Boston: Pearson, 2014, pp. 443--445.
...
{[}113{]} ``Basisleser weiterhin kritische schwachstelle des
...
{[}114{]} O. Stiemerling, ``Qualifizierte elektronische signatur mit dem
...
{[}115{]} ``BSI - technische richtlinien des BSI - BSI TR-03130
...
{[}116{]} ``Personalausweisportal - eID-server.'' {[}Online{]}.
Available:
...
{[}117{]} K. Nguyen and C. Schwarz, ``Innovatives key management für die
...
{[}118{]} D. B. der Bundesregierung für Informationstechnik,
...
{[}119{]} L. Neumann, ``Stellungnahme zum elektronischen
