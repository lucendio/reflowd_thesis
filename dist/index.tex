...
system provides it's own \emph{PKI}. Whereas a token-based
authentication on top of \emph{HTTPS} and public CAs should be suitable
for communication between the system and the \emph{operator}, preferable
based on \emph{\protect\hyperlink{link_jwt}{JSON Web Tokens}}, because
the session state is preserved within the token rather then having the
...
replace the approach consisting of \emph{mutual authentication} on TLS
...
not, a HTTPS connection will establish in any case. To hardening an
...
it is also conceivable, to \ldots{} instead of
...
The conglomeration below highlights all major components, that are
required in order to cover most of the defined use cases. It includes
further details about their purpose(s) and relation(s) to each other.

\subparagraph{Web server}\label{web-server}

\emph{Purpose:} + serve web-based user interface(s) + handle all in- \&
outgoing traffic (outmost layer) + revers proxying certain traffic to
different components + en- \& decrypt HTTPS traffic, thus authenticate
\emph{consumers} + load balancing (if necessary) + web client
notification \emph{Platform:} server \emph{Technologies:} + HTTP + TLS +
WebSockets \emph{Explanation/argumentation:}

\subparagraph{Permission Manager}\label{permission-manager}

\emph{Purpose:} * creating \emph{permission profiles} + permission
validation + examine data queries + queue \emph{consumer} requests
\emph{Platform:} server \emph{Technologies:}

\subparagraph{PKI}\label{pki}

\emph{Purpose:} + CA + manage keys and certificates per \emph{endpoint}
\emph{Platform:} server \emph{Technologies:} + X.509

\subparagraph{Storage Connector}\label{storage-connector}

\emph{Purpose:} + abstracts to system agnostic Query Language + queries
personal data, regardless of where it's located \emph{Platform:} server
\emph{Technologies:} + driver for used database

\subparagraph{Operator API}\label{operator-api}

\emph{Purpose:} + authenticates \emph{operator} + writes personal data
through Storage Connector + provides relevant data, such as history +
system configuration \emph{Platform:} server \emph{Technologies:} + JWT

\subparagraph{Code Execution
Environment}\label{code-execution-environment}

\emph{Purpose:} + isolated runtime (sandbox) for computations/programs
provided by \emph{consumers} + restrict interaction with outer
environment to absolute minimum (e.g.~no shared filesystem or network) +
one-time use + monitor sandbox during computation + examine and test the
provided software \emph{Platform:}\\
\emph{Technologies:} + Virtualization + Container (OCI)

\subparagraph{Tracker}\label{tracker}

\emph{Purpose:} + log all changes made with \emph{Storage Connector} +
tracks states for ongoing consumer requests + log all \emph{access
requests} \emph{Platform:} server \emph{Technologies:}

\subparagraph{Personal Data Storage}\label{personal-data-storage}

\emph{Purpose:} + stores the \emph{operator's} personal data
\emph{Platform:} server, mobile \emph{Technologies:} + non relational
database + depending on host environment

\subparagraph{Persistence Layer}\label{persistence-layer}

\emph{Purpose:} + stores Permission Profiles, History, Tokens,
Configurations and other application data + cache runtime data and
information \emph{Platform:} server \emph{Technologies:} + non
relational database + Filesystem

\subparagraph{Notification
Infrastructure}\label{notification-infrastructure}

\emph{Purpose:} + notify about everything that needs \emph{operator's}
approval (e.g.~new registrations, new \emph{permission requests})
\emph{Platform:} server \emph{Technologies:} + WebSockets for web UIs
via local web server + mobile device manufacturer's Push Notification
server for mobile apps

\subparagraph{User interface}\label{user-interface}

\emph{Purpose:} + access restricted to \emph{operator} only + access \&
permission management + data management (editor, types \& import) +
history and log viewer + system monitoring \emph{Platform:} desktop,
mobile \emph{Technologies:} + HTML, CSS, Javascript + Java + Swift,
Objective-C

After defining all different components portability
(\protect\hyperlink{sa02}{S.A.02})
...
TODO: does every component need to provide HTTP as their components? how
does auth/security work then?

TODO: it is debatable whether to place the \emph{permission profiles} in
the \emph{persistence layer} among all other domain-related information,
put it into the \emph{personal data storage} or define it as an own
storage component, in order to be flexible regarding it's location.

TODO: OpenID as auth services (like fb for other platforms) As discussed
in previous sections the \emph{PDaaS} has it's qualities to be seen and
valued as a digital representation of the individual that operates that
system. Hence it could also be used to authenticate the individual
against an external platform. Although it is priorly argued, that for
authentication and security reasons the system is designed to be
self-contained. But providing those OpenID capabilities isn't something
the \emph{PDaaS} depends on, instead external platforms would depend on
the \emph{PDaaS} supporting that feature. It is imaginable, that an
OpenID instance mapping on a single individual would simplify the
authentication procedure provided by that technology. However, it is
beyond the scope of this work to either do further investigations
regarding that topic, nor elaborate on how this might look integrate
with the currently developed feature-set.
...
\item
  ...
  \tightlist
  \item
    on grant, response includes certificate and information on what data
    points are allowed to be accessed
  \item
